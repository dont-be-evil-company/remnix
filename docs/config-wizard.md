...
Protected deletes: `/`, drive root, `$HOME`, syncsh config/data roots.
...
After a selection, if the path is not already a syncsh repository, the wizard
offers to create a `syncsh` subfolder so other Drive files are not scanned.

Join still requires `repository.Probe == Valid`. Creating a brand-new folder
on a second device is not enough - run `syncsh setup` on the first device, or
