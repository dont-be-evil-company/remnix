...
func TestIntegrationSupported(t *testing.T) {
	...
	for _, sh := range []string{"zsh", "bash", "fish", "nu"} {
		out, err := Integration(sh, "remnix", opts)
		if err != nil {
			t.Fatalf("%s: %v", sh, err)
		}
		...
		hasZshRPC := strings.Contains(out, "__remnix_rpc start")
		if !hasLegacy && !hasRPC && !hasZshRPC {
			t.Fatalf("%s hook missing lifecycle commands", sh)
		}
		if sh == "zsh" && !strings.Contains(out, `"$__remnix_bin" daemon`) {
			t.Fatal("zsh hook must start the daemon")
		}
		if sh == "bash" && !strings.Contains(out, `__remnix_agent_ensure`) {
			t.Fatal("bash hook must start the history agent")
		}
		if sh == "fish" && !strings.Contains(out, `__remnix_agent_ensure`) {
...
func TestCtrlRBindings(t *testing.T) {
	opts := Options{SuggestEnabled: true, SuggestAccept: []string{"Right"}}
	zsh, _ := Integration("zsh", "remnix", opts)
	if !strings.Contains(zsh, "bindkey '^R'") {
		t.Fatal("zsh missing bindkey ^R")
	}
	if !strings.Contains(zsh, "zle .accept-line") || !strings.Contains(zsh, "__remnix_accept__:") {
	...
	if clearAt < 0 || redisplayAt < 0 || invalidateAt < 0 || overlayAt < 0 || tuiAt < 0 {
		t.Fatal("remnix-search missing clear/redisplay/zle -I before overlay/TUI")
	}
	if overlayAt < invalidateAt {
		t.Fatal("zle -I must precede daemon overlay RPC")
	}
	if !strings.Contains(searchFn, "__remnix_overlay search-interactive") {
		t.Fatal("remnix-search must try daemon overlay RPC first")
	}
	if !strings.Contains(searchFn, "__remnix_widget_run search --interactive") {
		t.Fatal("remnix-search must fall back to the local TUI via __remnix_widget_run")
	}
	...
	if strings.Contains(searchFn, `selected="$("$__remnix_bin" search --interactive`) {
		t.Fatal("remnix-search must not capture TUI stdout with command substitution")
	}
	if !strings.Contains(searchFn, "echoti ed") {
		t.Fatal("remnix-search must clear-to-eos after TUI in case alt-screen restored junk")
	}
	...
	if !strings.Contains(zsh, "__remnix_suggest_suppress") {
		t.Fatal("remnix-search must suppress suggest redraw during ctrl+r/accept")
	}
	...
	if !strings.Contains(zsh, `__remnix_suggest_suppress} )) && __remnix_suggest_suppress=0`) &&
		!strings.Contains(zsh, "suggest_suppress=0") {
		t.Fatal("precmd must clear suggest suppress after the command")
	}
	menu, _ := Integration("zsh", "remnix", Options{SuggestEnabled: true, SuggestMenu: true, SuggestAccept: []string{"Right"}})
	if !strings.Contains(menu, `__remnix_suggest_idx > 0`) {
		t.Fatal("multi-line suggest menu must only paint while navigating (idx>0)")
	}
	bashHook, _ := Integration("bash", "remnix", opts)
	...
	fishHook, _ := Integration("fish", "remnix", opts)
	if !strings.Contains(fishHook, `__remnix_bind \cr`) {
	...
	if strings.Contains(fishHook, "(__remnix_widget_run") {
	...
	if strings.Contains(nuHook, "source (remnix init") {
	...
	if !strings.Contains(nuHook, "source ~/.cache/remnix.nu") {
	...
	if !strings.Contains(nuHook, "__remnix_rebind") || !strings.Contains(nuHook, "where {|k|") {
		t.Fatal("nu must replace the default history_menu Ctrl+R, not only append")
	}
}

func TestUnsupported(t *testing.T) {
	if _, err := Integration("tcsh", "remnix", Options{}); err == nil {
		t.Fatal("expected error")
	}
}

func TestZshInlineSuggest(t *testing.T) {
	on, err := Integration("zsh", "remnix", Options{SuggestEnabled: true, SuggestAccept: []string{"Right", "Tab"}})
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
		...
		"__remnix_suggest_clear_then_$w",
	...
	if !strings.Contains(on, "__remnix_suggest_clear") || strings.Count(on, `BUFFER="$BUFFER$POSTDISPLAY"`) != 1 {
		t.Fatal("Enter must drop ghost text; only the accept widget may merge POSTDISPLAY")
	}
	if strings.Contains(on, "typeset -gi __remnix_suggest_suppress=1") {
		t.Fatal("suppress must be assigned globally; typeset inside a widget shadows it")
	}
	if strings.Contains(on, `[[ ${widgets[$w]:-} == user:__remnix_suggest_clear_then_$w ]] && continue`) {
		t.Fatal("accept-line wrapper must be refreshed on re-eval, not skipped when already bound")
	}
	if strings.Contains(on, `__remnix_suggest_clear
        zle redisplay
        zle $orig`) {
	...
	if strings.Contains(on, "__remnix_suggest_orig[$WIDGET]") || strings.Contains(on, "__remnix_suggest_clear_then_orig") {
	...
	off, err := Integration("zsh", "remnix", Options{SuggestEnabled: false})
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(off, "suggest --prefix") || strings.Contains(off, "remnix-suggest-accept") || strings.Contains(off, "__remnix_rpc suggest") {
		t.Fatal("disabled suggest should not emit inline suggestion hooks")
	}
}

func TestZshSuggestMenu(t *testing.T) {
	off, err := Integration("zsh", "remnix", Options{SuggestEnabled: true, SuggestAccept: []string{"Right"}})
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(off, "suggest-list") || strings.Contains(off, "remnix-suggest-next") || strings.Contains(off, "┌") || strings.Contains(off, "#F5C2E7") {
		t.Fatal("menu should be omitted when disabled")
	}
	on, err := Integration("zsh", "remnix", Options{SuggestEnabled: true, SuggestMenu: true, SuggestAccept: []string{"Right"}})
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
		...
		"bindkey '^P'",
		"bindkey '\\e'",
		"__remnix_rpc suggest",
		"__remnix_menu_hl_sel",
		"__remnix_menu_hl_desc",
		"fg=#F5C2E7",
		"bg=#313244",
		"fg=#CDD6F4",
		"█",
	...
	if strings.Contains(on, "__remnix_suggest_kinds+=(history)") {
		t.Fatal("LSP menu must not list history rows")
	}
	if strings.Contains(on, "suggest-list") || strings.Contains(on, "__remnix_rpc_list") || strings.Contains(on, "--list") {
		t.Fatal("LSP menu must not fetch suggest-list")
	}
	if strings.Contains(on, "_main_complete") || strings.Contains(on, "__remnix_compadd") || strings.Contains(on, "zle -C __remnix_comp_list") {
...
func TestZshSuggestCompletions(t *testing.T) {
	...
	for _, want := range []string{
		...
		"__remnix_suggest_icon_completion",
		"__remnix_comp_values >= 512",
		"__remnix_comp_cache_key",
		"__remnix_comp_inserts",
		"__remnix_comp_cache_inserts",
		"__remnix_suggest_off",
		"█",
	...
	if strings.Contains(on, "__remnix_comp_values >= 64") {
	...
	if strings.Contains(on, "__remnix_suggest_fetch_hist") || strings.Contains(on, "suggest-list") {
		t.Fatal("completions menu must not fetch history rows")
	}
	if strings.Contains(on, "if (( ${+functions[__remnix_suggest_completions]} )); then") {
	...
	f, err := os.CreateTemp(t.TempDir(), "remnix-init-*.zsh")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPtyProxyPreamble(t *testing.T) {
	off, err := Integration("zsh", "/opt/remnix", Options{SuggestEnabled: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	on, err := Integration("zsh", "/opt/remnix", Options{SuggestEnabled: true, PtyProxyEnabled: true})
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(on, "exec '/opt/remnix-attach'") {
...
func TestZshOverlayMenuWhenProxy(t *testing.T) {
	post, err := Integration("zsh", "remnix", Options{SuggestEnabled: true, SuggestMenu: true, SuggestAccept: []string{"Right"}})
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(post, "__remnix_suggest_menu_box") {
	...
	over, err := Integration("zsh", "remnix", Options{
		SuggestEnabled:  true,
		SuggestMenu:     true,
	...
	if strings.Contains(over, "__remnix_suggest_menu_box") {
		t.Fatal("proxy-on must not emit POSTDISPLAY menu")
	}
	if !strings.Contains(over, `_main_complete`) || !strings.Contains(over, `__remnix_comp_applied_lines`) {
	...
	if strings.Contains(over, "bindkey $'\\t' remnix-suggest-complete") {
	...
	if !strings.Contains(over, "__remnix_overlay_complete_begin") {
		t.Fatal("proxy-on Ctrl+Space must open the overlay before compsys so a spinner can paint")
	}
	menuAt := strings.Index(over, "remnix-suggest-menu()")
	if menuAt < 0 {
		t.Fatal("missing remnix-suggest-menu")
	}
	menu := over[menuAt:]
	beginAt := strings.Index(menu, "__remnix_overlay_complete_begin")
	compAt := strings.Index(menu, "__remnix_suggest_completions")
	if beginAt < 0 || compAt < 0 || beginAt > compAt {
		t.Fatal("spinner overlay must start before compsys capture")
	}
	if !strings.Contains(over, "__remnix_rpc_write") || !strings.Contains(over, "__remnix_overlay_complete_finish") {
...
func TestBashBleGhostAndOverlayMenu(t *testing.T) {
	out, err := Integration("bash", "remnix", Options{SuggestEnabled: true, SuggestMenu: true})
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(out, "ble/complete/auto-complete/source:remnix") {
		t.Fatal("bash should register a ble.sh autosuggest source")
	}
	if !strings.Contains(out, `__remnix_rpc suggest`) {
...
func TestFishOverlayMenuNoGhost(t *testing.T) {
	out, err := Integration("fish", "remnix", Options{SuggestEnabled: true, SuggestMenu: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestNuOverlayMenu(t *testing.T) {
	out, err := Integration("nu", "remnix", Options{SuggestMenu: true})
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(out, "remnix-suggest-menu") || !strings.Contains(out, "keycode: space") {
...
func TestFishInitParses(t *testing.T) {
	...
	out, err := Integration("fish", "/opt/remnix", Options{
		SuggestEnabled:  true,
		SuggestMenu:     true,
		PtyProxyEnabled: true,
	})
...
func TestNuInitParses(t *testing.T) {
	...
	out, err := Integration("nu", "/opt/remnix", Options{
		SuggestEnabled:  true,
		SuggestMenu:     true,
		PtyProxyEnabled: true,
	})
