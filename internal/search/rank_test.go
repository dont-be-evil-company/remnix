...
func TestRankPrefersSubstringOverScatteredLetters(t *testing.T) {
	entries := []history.Entry{
		...
		{Command: "remnix daemon uninstall", StartTS: time.Unix(50, 0)},
		{Command: "remnix daemon install", StartTS: time.Unix(51, 0)},
		{Command: "remnix daemon status", StartTS: time.Unix(60, 0)},
	}
	...
	for i, want := range []string{
		...
		"remnix daemon install",
		"remnix daemon uninstall",
	} {
...
func TestRankMultiTermRequiresAll(t *testing.T) {
	entries := []history.Entry{
		{Command: "remnix daemon status", StartTS: time.Unix(2, 0)},
		{Command: "systemctl status nginx", StartTS: time.Unix(3, 0)},
	}
	got := Rank("remnix daemon", entries, false)
	if len(got) != 1 || got[0].Entry.Command != "remnix daemon status" {
		t.Fatalf("got %+v", got)
	}
}
...
func TestMinSpan(t *testing.T) {
	span, ok := minSpan([]rune("daemon"), []rune("remnix daemon uninstall"))
	if !ok || span != 6 {
