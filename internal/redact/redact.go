...
var (
	kv   = regexp.MustCompile(`(?i)(password|pass|token|refresh_token|access_token|client_secret|secret_access_key|sas_url|connection_string|service_account_credentials|cookie|bearer_token|SYNCSH_RECOVERY_KEY|RCLONE_CONFIG_PASS|AWS_SECRET_ACCESS_KEY)\s*[=:]\s*(\S+)`)
	bech = regexp.MustCompile(`syncsh1[0-9a-z]+`)
)

func String(s string) string {
	s = kv.ReplaceAllString(s, "$1=***")
	s = bech.ReplaceAllString(s, "syncsh1***")
	return s
}
